...
def get_documents_root() -> str:
    """
    Get the base directory for Word documents.

    Returns:
        str: The base directory path for documents
    """
    return os.environ.get("WORD_DOCUMENTS_ROOT", "./documents")


def resolve_document_path(name_or_path: str) -> str:
    """
    ...
    - If only a file name, place it under the documents root directory.
    Ensures the root directory exists.

    Args:
        name_or_path: User-supplied document identifier
    ...
    try:
        ...
        if os.path.isabs(raw) or os.path.dirname(raw):
            ...
            return abs_path
        root = os.path.abspath(get_documents_root())
        os.makedirs(root, exist_ok=True)
        return os.path.join(root, raw)
    except Exception as e:
        print(f"Error resolving document path: {e}")
        raise
